...
class AdaptiveDataLoaderHelper(object):
    ...
    def _sync_local_bsz(self):
        ...
        elif self._state.current_local_bsz in [1, 2, 4, 8]:
    ...
    # the below profiler is not needed for now since batch size sync across replicas (GPUs) is managed by Alpa
    ...
    #     self.future_exit = alpa.adaptdl.collective.allreduce_async(
...
def _worker_init_wrapper(worker_init_fn, num_workers):
    ...
    def wrapper(worker_id):
        ...
        seed = torch.initial_seed() + alpa.adaptdl.env.replica_rank() * num_workers
...
class _AdaptiveDataLoaderState(alpa.adaptdl.checkpoint.State):
    ...
    def __init__(self):
        ...
        LOG.info(f"Adaptive dataloader state name - {'adaptdl-dataloader-epoch{}-{}'.format(epoch, count)}")
